...
import com.Farmacia.service.ProductService;
...
//		    	e.printStackTrace();
	...
	public String loanProductsUser(@RequestParam("id") long id, Model model) {
		User user = userService.getUser(id);
		model.addAttribute(user);
		long miArray[] = new long[user.getProducts().size()]; 
		for(int i = 0; i < user.getProducts().size(); i++) {
			miArray[i] = user.getProducts().get(i).getId(); 
		}
		model.addAttribute("miArray", miArray);
		//model.addAttribute("productlist", productService.listProducts());
		
		model.addAttribute("titulo", "Listado de productos prestados a " + user.getName());
		model.addAttribute("descripcion", "Formulario para añadir/modificar productos prestados por " + user.getName());
		return "loanProducts";
	}
