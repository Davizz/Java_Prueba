...
import com.Farmacia.config.HibernateUtil;
...
//			query.setParameter("cadena", "%de%");
			...
			query = session.createQuery("from Product product where product.author=:name ");
			query.setParameter("name", "François Rabelais");
			list = query.list();
			sacaLista(list);
			...
			query = session.createQuery("select b from Product p where p.title like :cadena").setParameter("cadena",
					"%es%");
			list = query.list();
			sacaLista(list);
			System.out.println("****************PASAR UN SELECT A MAYUSCULAS*************************");

			query = session.createQuery("select upper(p.title) from Product p where p.title like :cadena")
					.setParameter("cadena", "%es%");
			list = query.list();
			sacaListaSelect(list);
			System.out.println("****************PASAR UN SELECT CON VARIAS COLUMNAS*************************");

			query = session.createQuery("select new List(p.title, p.author, p.id) from Product p");
			list = query.list();
			sacaListaSelect(list);
			/* USO DE IN() */
			System.out.println(
					"****************SACAR UNA SERIE DE TÍTULOS SEGÚN UNA LISTA DE AUTORES*************************");
			query = session.createQuery("from Product p where p.author in('George Sand','François Rabelais')");
			list = query.list();
			sacaLista(list);
			/* ORDER BY */
			System.out.println(
			...
			query = session
					.createQuery("from Product p where p.author in('George Sand','François Rabelais') ORDER BY p.title");
			list = query.list();
			sacaLista(list);
			/* NOT NULL Y ASÍ INCLUIR LA CLASE CATEGORY */
			System.out.println("*****************SACAR LOS IS NOT NULL************************");
			query = session.createQuery("from Product p where p.category IS NOT NULL");
			list = query.list();
			sacaLista(list);
			/* USO DE GROUP BY */
			System.out.println("****************AGRUPAR la consulta*************************");
			query = session.createQuery("from Produt p where p.category IS NOT NULL GROUP BY p.category");
			list = query.list();
			sacaGrupo(list);
			...
			query = session.createQuery(
					"from Product p where p.bookDetails.purchase_year = (select max(bd.purchase_year) FROM ProductDetails bd)");
			list = query.list();
			sacaLista(list);
			...
			query = session.createQuery("SELECT product FROM Product product inner join product.users user ORDER BY user.id asc");
			list = query.list();
			sacaLista(list);
