...
public class CategoryController {
	...
	public String editCategory(@RequestParam("id") long id, Model model) {	
		...
		addAttributes(model, "Formulario Categoías", "Formulario para añadir/modificar las categorías");
		return "categoryList";
	}
	...
	@PostMapping("/save")
	   public String saveCategory(@ModelAttribute("category") @Valid Category category,
	      ...
	      if (result.hasErrors()) {
	    	  addAttributes(model, "Formulario de categorías", "Formulario para añadir/modificar las categorías"); 
	         //model.addAttribute("categories", categoryService.listCategories());
	         return "categoryList";
	      }
